    ...
    if request.method == 'POST':
        ...
        if is_doctor:
            ...
            if result:
                ...
                with connection.cursor() as cursor:
                    cursor.execute("""
                        SELECT fname, lname, id, phone_number 
            ...
            if result:
                ...
                with connection.cursor() as cursor:
                    cursor.execute("""
                        SELECT fname, lname, id, bloodtype, phone_number 
                        FROM patient WHERE id = %s
                    """, [patient_id])
                    cols = [col[0] for col in cursor.description]
                    patient = dict(zip(cols, cursor.fetchone()))

                # Fetch diagnosis from patient_history
                with connection.cursor() as cursor:
                    cursor.execute("SELECT diagnosis FROM patient_history WHERE id = %s", [patient_id])
                    history_result = cursor.fetchone()
                    if history_result:
                        patient['diagnosis'] = history_result[0]
                    else:
...
def signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if not all([email, password, birthdate, gender, fname, lname,phone_number]):
